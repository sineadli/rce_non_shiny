...
var getSelectedEvaluations = function (req, res, next) {
    ...
    if (search) {
        ...
        Evaluation.find().sort({ "basics.Basics_Tech_Name": sort }).select("userid title basics.Basics_Tech_Name planContext published_at author company").exec(function (err, evals) {
            if (err) {
                console.log(err);
                return next();
            } else {
